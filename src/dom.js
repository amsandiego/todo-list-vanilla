    ...
    if (modalAction === 'Delete') {
      ...
      if (modalTask === 'Delete Project') {
        ...
        projectDeletionTitle.textContent =
          projects.projectList[projectIndex].title;
  ...
  const showTasks = (menuTitle, projectIndexStart, projectIndexEnd) => {
    ...
    for (let i = projectIndexStart; i < projectIndexEnd; i++) {
      for (let j = 0; j < projects.projectList[i].tasks.length; j++) {
        ...
        if (
          menuTitle === 'important' &&
          projects.projectList[i].tasks[j].priority !== 'high'
        ) {
          continue;
          ...
          const dateOfToday = parseISO(today);
          ...
          menuTitle === 'completed' &&
          projects.projectList[i].tasks[j].completed !== true
        ) {
          continue;
        }
